...
const mockCobradores = [
  {
    ...
    name: "Juan Pérez",
    phone: "809-555-0001",
    email: "juan.perez@nmv.com",
    zone: "Zona Norte",
    bancas: 8,
    status: "activo",
    balance: 12500,
    lastCollection: "2026-06-17",
  },
  {
    id: "COB-002",
    name: "María García",
    phone: "809-555-0002",
    email: "maria.garcia@nmv.com",
    zone: "Zona Sur",
    bancas: 6,
    status: "activo",
    balance: 8750,
  ...
  },
  {
    ...
    name: "Luis Rodríguez",
    phone: "809-555-0005",
    email: "luis.rodriguez@nmv.com",
    zone: "Centro",
    bancas: 7,
    status: "activo",
    balance: 6800,
    lastCollection: "2026-06-16",
  },
];
...
export default function ListaCobradores() {
  ...
  const filtered = useMemo(() => {
    return mockCobradores.filter((c) => {
      const matchesSearch =
  ...
  const activos = mockCobradores.filter((c) => c.status === "activo").length;
  const totalBalance = mockCobradores.reduce((sum, c) => sum + c.balance, 0);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Total Cobradores", value: mockCobradores.length, color: "#14B8A6" },
          { label: "Activos", value: activos, color: "#10B981" },
          { label: "Inactivos", value: mockCobradores.length - activos, color: "#94A3B8" },
          {
            label: "Balance Total",
      ...
      <div className="bg-white rounded-xl border border-[#E5E5E0] shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#F8F8F5] text-[#666666] text-[12px] font-semibold uppercase tracking-[0.04em]">
                  ...
                  <th className="px-4 py-3 text-left border-b border-[#E8E8E3]">Contacto</th>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-[#999999] text-sm">
                  ...
                  filtered.map((cobrador, idx) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <span className="font-mono text-[12px] font-semibold text-[#14B8A6]">
                          {cobrador.id}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="flex items-center gap-1 text-[12px] text-[#666666]">
                            <Mail size={11} className="text-[#999999]" />
                            {cobrador.email}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-[#F59E0B]">
                        ${cobrador.balance.toLocaleString()}
                      </td>
                      <td className="px-4 py-3 text-center">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button className="p-1.5 rounded-md hover:bg-red-50 text-[#666666] hover:text-red-500 transition-colors" title="Eliminar">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </td>
            ...
            {filtered.map((cobrador, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-mono text-[11px] font-semibold text-[#14B8A6]">
                      {cobrador.id}
                    </p>
                ...
                <div className="space-y-1.5 mb-3">
                  ...
                  <div className="flex items-center gap-1.5 text-[12px] text-[#666666]">
                    <Mail size={11} className="text-[#999999]" /> {cobrador.email}
                  </div>
                  <div className="flex items-center gap-1.5 text-[12px] text-[#666666]">
                    <MapPin size={11} className="text-[#8B5CF6]" /> {cobrador.zone}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-[#F0F0EC]">
                  <div className="text-center">
                    ...
                    <p className="font-bold text-sm text-[#333333]">{cobrador.bancas}</p>
                  </div>
                  <div className="text-center">
        ...
        <div className="px-4 py-3 border-t border-[#E5E5E0] bg-[#FAFAF8] flex items-center justify-between">
          <span className="text-[12px] text-[#999999]">
            {filtered.length} de {mockCobradores.length} cobradores
          </span>
        </div>
      </div>
    </motion.div>
